...
| decode GEMM | `grouped_b0_gemm_decode` / `_fp8` [HK] | **a *different* GEMM for decode: skinny BM=16 tile** (vs prefill's 256) so small experts don't grind padding; **native fp8** via store-fp8/load-as-bf16/unpack + offline pre-swizzle. 3.97 TB/s standalone. Critical: a `s_waitcnt lgkmcnt(0)` + `sched_barrier(0)` before the mma (compiler was hoisting the mma above the data wait). | `num_tasks`, 512 thr |
