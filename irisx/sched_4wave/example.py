#!/usr/bin/env python3
# fmoe_sched_4wave / example.py
# ------------------------------------------------------------------------------------------------
# Symmetric 4-wave LATENCY-path head-to-head driver (np=2). Same ABI/numerics as V4.
#   Rank 0 ("source rank") holds the ONLY real copy of QUANTIZED fp8 activations A_fp8[M,K] + their
#     per-128-group fp32 scales A_sc[M,K/128] on its IRIS heap.
#   Rank 1 ("consumer rank") runs the expert GEMM C = dequant(A_fp8) @ B^T. Its kernel's 4 SYMMETRIC
#     waves each gather their own A-row strip over IRIS, dequantize, and MFMA — a per-wave software
#     pipeline (FUSED) vs a two-phase gather-then-MFMA (BASELINE). Same shapes/layout/gather/dequant;
#     only the schedule differs.
#
# This kernel targets SMALL M (M<=256) where V4's producer/consumer split loses. Sweep M small.
# Reports: correctness (RMS-rel vs bf16 ref) + zero-sentinel for both, and head-to-head wall-clock.
# ------------------------------------------------------------------------------------------------
import sys, os, time
sys.path.insert(0, "..")
import torch
...
# ---- shapes (override via env). Small-M default for the latency path. ----
...
# allocate M*K BYTES (as bf16 [M,K//2]) on the heap. TWO views over the SAME storage:
#   - A_fp8_bf16 [M,K//2] bf16  : tensor PASSED to the kernel (gl<bf16> reinterprets bytes)
...
if rank != SRC_RANK:
    ...
    print(f"  FUSED    (4-wave interleave)     : {t_fused:8.2f} us/iter   {flops/(t_fused*1e-6)/1e12:6.2f} TFLOP/s", flush=True)
    spd = t_base / t_fused
