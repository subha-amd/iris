#!/usr/bin/env python3
# fmoe_fused_sched_xcd / example.py
# ------------------------------------------------------------------------------------------------
# SCAFFOLD for the MAIN AGENT (subagents do NOT run this — it touches the GPU).
#
# Driver identical to V4's (np=2 head-to-head), so the only experimental variable is the kernel's
# XCD block-remap (compile-time XCD_REMAP / XCD_W / XCD_C in kernel.cpp).  To A/B test the L2-reuse
# hypothesis, build the kernel twice and run this driver against each build:
#   build A:  default (XCD_REMAP=1)  -> XCD-aware co-location of an M-tile's A-sharing superblocks
#   build B:  -DXCD_REMAP=0          -> exact stock V4
# Correctness MUST be identical (RMS-rel 0.00331, zero-sentinel) in both — the remap is a pure
# permutation.  The interesting signal is in rocprof-compute XGMI / L2-TCC / LLC counters, NOT in
# wall-clock alone (see AGENT_REPORT.md for the exact counter command).
#
#   Rank 0 holds the ONLY real copy of quantized fp8 A[M,K] + per-128 fp32 scales on its IRIS heap.
#   Rank 1 runs the expert GEMM C = dequant(A_fp8) @ B^T; its producer warps pull fp8 A over IRIS.
# ------------------------------------------------------------------------------------------------
import sys, os, time
sys.path.insert(0, "..")
import torch
...
# ---- shapes (override via env). Canonical XCD A/B point: M=1024 K=7168 N=2048. ----
...
if rank != SRC_RANK:
    ...
    print(f"  FUSED-XCD (gather+dequant+MFMA)  : {t_fused:8.2f} us/iter   {flops/(t_fused*1e-6)/1e12:6.2f} TFLOP/s", flush=True)
    spd = t_base / t_fused
