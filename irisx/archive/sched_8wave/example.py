#!/usr/bin/env python3
# sched_8wave / example.py  (HK 8-wave ping-pong role-swap driver, np=2)
# ------------------------------------------------------------------------------------------------
# Same head-to-head harness as irisx/v4_astationary_kernel/example.py and the SAME metadata ABI:
...
#   Rank 1 ("consumer rank") runs the expert GEMM C = dequant(A_fp8) @ B^T using the 8-wave
#     PING-PONG kernel: two symmetric wavegroups, each gathers its OWN disjoint A row-half over
#     IRIS, dequantizes, MFMAs against local-HBM B, and stores DISJOINT output rows (no partial-K
#     reduction).  fused=1 = role-swap ping-pong; fused=0 = two-phase baseline (no overlap).
#
# Correctness: RMS-rel err vs bf16 reference; zero-sentinel proves the remote gather is real.
# Head-to-head: ping-pong vs baseline wall-clock + TFLOP/s.
#
# NOTE: This is a SCAFFOLD for the main agent.  NO GPU was run here.  Run command in AGENT_REPORT.md.
# ------------------------------------------------------------------------------------------------
import sys, os, time
sys.path.insert(0, "..")
import torch
...
# ---- shapes (override via env). K=7168 for R1 W13 input. ----
...
# fp8 buffer over bf16 storage (HK gl rejects 1-byte elem types); two views over SAME storage.
...
if rank != SRC_RANK:
    ...
    print(f"  PINGPONG (8-wave role-swap)      : {t_fused:8.2f} us/iter   {flops/(t_fused*1e-6)/1e12:6.2f} TFLOP/s", flush=True)
    spd = t_base / t_fused
    print(f"  SPEEDUP (baseline/pingpong)      : {spd:6.3f}x   ({'PINGPONG WINS' if spd>1 else 'baseline wins'})", flush=True)
    print("="*78, flush=True)
