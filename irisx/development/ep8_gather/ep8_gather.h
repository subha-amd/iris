...
static constexpr signed char SEG_NONE = -1;   // row_seg sentinel: dst row has no source segment
...
__device__ __forceinline__ void build_row_seg_map(
        signed char (&row_seg)[BM], const seg_tile_view& v, int tid, int nthreads) {
    ...
    for (int si = 0; si < v.seg_count; ++si) {
        ...
        for (int r = lo + tid; r < hi; r += nthreads)
            row_seg[r] = (signed char)(v.seg_begin + si);     // absolute index into segs[]
    }
    __syncthreads();
}
...
__device__ __forceinline__ void gather_dequant_A_tile_multisource(
    ...
    for (int ci = tid; ci < total_chunks; ci += nthreads) {
        ...
        if (fast_single) {
            ...
            const signed char sidx = row_seg[r];
            if (sidx == SEG_NONE) {
